...
EOF
    ...
    cat > "$repo/assets/maestro-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/maestro/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/maestro/.fixture-keep"
    cat > "$repo/plugins/maestro/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/maestro/.fixture-keep
    git -C "$repo" add plugins/maestro/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/maestro/skills/example/agents"

    cat > "$repo/plugins/maestro/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/maestro/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/maestro/.codex-plugin" \
        "$repo/plugins/maestro/.private-journal" \
        "$repo/plugins/maestro/assets" \
        "$repo/plugins/maestro/hooks" \
        "$repo/plugins/maestro/skills/example/agents" \
        "$repo/plugins/maestro/skills/example"

    cat > "$repo/plugins/maestro/.codex-plugin/plugin.json" <<EOF
{
  "name": "maestro",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/maestro/assets/maestro-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/maestro/assets/app-icon.png"

    cat > "$repo/plugins/maestro/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/maestro/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/maestro/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/maestro/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/maestro/hooks/session-start" "$repo/plugins/maestro/hooks/session-start-codex" "$repo/plugins/maestro/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/maestro/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/maestro/.codex-plugin/plugin.json \
        plugins/maestro/assets/app-icon.png \
        plugins/maestro/assets/maestro-small.svg \
        plugins/maestro/hooks/hooks-codex.json \
        plugins/maestro/hooks/run-hook.cmd \
        plugins/maestro/hooks/session-start \
        plugins/maestro/hooks/session-start-codex \
        plugins/maestro/skills/example/agents/openai.yaml \
        plugins/maestro/skills/example/SKILL.md \
        plugins/maestro/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/maestro/.kimi-plugin" \
        "$repo/plugins/maestro/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/maestro/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/maestro/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/maestro/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/maestro/.fixture-keep \
        plugins/maestro/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/maestro/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/maestro/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/maestro-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (maestro-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/maestro-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/maestro/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/maestro" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/maestro-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/maestro'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/maestro-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/maestro-*" "Clean no-op local apply does not create sync branch in destination checkout"
