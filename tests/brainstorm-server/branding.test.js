/**
 * Tests for the visual companion's Maestro/Prime Radiant branding.
 */
...
const ASSET_URL = 'https://primeradiant.com/brand/maestro-visual-brainstorming-logo.png';
...
function createPackagedServerFixture(version) {
  const root = fs.mkdtempSync(path.join('/tmp', 'maestro-packaged-server-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'maestro', version }, null, 2)
  );
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Maestro v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Maestro v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Maestro v/.test(html),
    'visible logo should appear before the Maestro version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Maestro v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Maestro branding'
  );
}
  ...
  await test('packaged Codex plugin reads version from .codex-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes('Maestro vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
