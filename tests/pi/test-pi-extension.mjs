...
const extensionPath = resolve(repoRoot, '.pi/extensions/maestro.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-maestro/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'maestro');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/maestro.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have maestro/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have maestro/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have maestro/);
