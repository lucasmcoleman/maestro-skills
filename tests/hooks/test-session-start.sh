...
mkdir -p "$legacy_home/.config/maestro/skills"
assert_command_output \
    "SessionStart omits obsolete legacy custom-skill warning" \
    "nested" \
    "" \
    "Maestro now uses"$'\037'"~/.config/maestro/skills"$'\037'"~/.claude/skills"$'\037'"legacy" \
    "$legacy_home" \
    CLAUDE_PLUGIN_ROOT="$REPO_ROOT" \
    bash "$HOOK_UNDER_TEST"
...
mkdir -p "$codex_legacy_home/.config/maestro/skills" "$codex_legacy_data"
assert_command_output \
