...
main() {
    ...
    if [[ "$dir" == "$repo/.maestro/sdd" ]]; then
        pass "prints <repo-root>/.maestro/sdd"
    else
        fail "prints <repo-root>/.maestro/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.maestro/sdd/.gitignore" && "$(cat "$repo/.maestro/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.maestro/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.maestro/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.maestro/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.maestro/sdd" && "$wt_dir" != "$dir" ]]; then
