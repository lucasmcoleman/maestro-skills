...
# 1. Create in maestro location (lowest priority)
mkdir -p "$SUPERPOWERS_SKILLS_DIR/priority-test"
cat > "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Maestro version of priority test skill
---
# Priority Test Skill (Maestro Version)
...
if [ -f "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Maestro version exists"
else
    echo "  [FAIL] Maestro version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled maestro priority
echo ""
echo "Test 2: Documenting personal vs maestro priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled maestro version instead of personal version"

# Test 3: Document project vs bundled maestro priority
echo ""
echo "Test 3: Documenting project vs personal/maestro priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled maestro version instead of project version"

# Test 4: Test a non-colliding bundled maestro skill is still available
echo ""
echo "Test 4: Testing non-colliding maestro skill remains available..."

mkdir -p "$SUPERPOWERS_SKILLS_DIR/maestro-only-test"
cat > "$SUPERPOWERS_SKILLS_DIR/maestro-only-test/SKILL.md" <<'EOF'
---
name: maestro-only-test
description: Maestro-only priority test skill
---
# Maestro Only Test Skill

PRIORITY_MARKER_SUPERPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"maestro-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERPOWERS_ONLY_VERSION" "Non-colliding maestro skill is still registered"
