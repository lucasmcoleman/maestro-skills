#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the maestro plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/maestro.js"
...
# Test 3: Check using-maestro skill exists (critical for bootstrap)
echo "Test 3: Checking using-maestro skill (required for bootstrap)..."
if [ -f "$SUPERPOWERS_SKILLS_DIR/using-maestro/SKILL.md" ]; then
    echo "  [PASS] using-maestro skill exists"
else
    echo "  [FAIL] using-maestro skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/maestro/' "$SUPERPOWERS_PLUGIN_FILE"; then
