...
A real integration loads the `using-maestro` bootstrap at session start. The bootstrap is what causes skills to auto-trigger at the right moments. Without it, the skills are dead weight — present on disk but never invoked.
...
- Use `maestro:writing-skills` to develop and test changes
